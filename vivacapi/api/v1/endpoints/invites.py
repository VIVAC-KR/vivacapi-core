...
@router.get("/{uid}", response_model=InvitePreview)
async def preview_invite(
    uid: str,
    session: AsyncSession = Depends(get_db),
) -> InvitePreview:
    """로그인 전에도 열람 가능한 초대장 미리보기."""
...
@router.post("/{uid}/accept", response_model=InviteResponse)
async def accept_invite(
    uid: str,
    user: User = Depends(get_current_user),
    session: AsyncSession = Depends(get_db),
) -> InviteResponse:
    """이미 가입된 유저가 그룹 초대 링크를 열었을 때 사용. 신규 가입자는 /auth/google이 처리한다."""
    invite = await crud_invite.get_invite_by_uid(session, uid)
    if invite is None:
        raise AppException(ErrorCode.INVITE_NOT_FOUND, "Invite not found")
